...
export function CricketMatchRow({ match, className }: CricketMatchRowProps) {
  ...
  return (
    <Link
      ...
      className={cn(
        "group flex items-center gap-4 px-5 py-4 panel panel-hover",
        match.status === "live" && "ring-1 ring-border-live/40",
        className
      )}
    >
      <div className="flex min-w-0 flex-1 flex-col gap-2">
        <div className="flex min-w-0 items-center gap-2">
          <span className="label shrink-0  border border-border-navy bg-blue/20 px-1.5 py-0.5 text-[10px] font-bold tracking-wider text-muted-strong rounded-sm">
            {format.shortLabel}
          </span>
          <span className="label truncate text-muted-strong">{match.series}</span>
        ...
        <div className="grid grid-cols-1 gap-x-4 gap-y-1 sm:grid-cols-2">
          {home && (
            <div className="flex items-center justify-between gap-3">
              <span className="truncate text-sm font-semibold text-foreground-soft">{home}</span>
          ...
          {away && (
            <div className="flex items-center justify-between gap-3">
              <span className="truncate text-sm font-semibold text-foreground-soft">{away}</span>
              <span className="shrink-0 text-sm font-bold tabular-nums text-foreground">
        ...
        {match.statusText && (
          <p className="meta truncate text-xs text-secondary">{match.statusText}</p>
        )}
      </div>
